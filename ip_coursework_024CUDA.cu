...
/******************************************************************************
  ...
  To compile adapt the code below wo match your filenames:
    nvcc -o ip_coursework_024 ip_coursework_024.c -lglut -lGL -lm
...
__global__ void detect_edges(float *in, float *out) {
  int i;
...
int main(int argc, char **argv) {
  ...
  float *in;
  float *out;
  cudaMalloc((void**) &in, arrayBytes);
  cudaMalloc((void**) &out, arrayBytes);

  struct timespec start, finish;
  long long int time_elapsed;
  clock_gettime(CLOCK_MONOTONIC, &start);

  detect_edges<<<720,7200>>>(out, in);

  cudaThreadSynchronize();
  cudaMemcpy(h_in, in, arrayBytes, cudaMemcpyDeviceToHost);
  cudaFree(in);
  cudaFree(out);
